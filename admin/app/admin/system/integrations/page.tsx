...
export default function IntegrationsPage() {
  ...
  return (
    <div className="space-y-6 text-foreground animate-fade-in pb-12 font-semibold text-xs">
        ...
        {navLinks.map(({ label, href }) => (
          <Link key={href} href={href}
            className={`px-3 py-2 rounded-t-lg transition-all border-b-2 ${
              href === "/admin/system/integrations" ? "border-primary text-primary bg-primary/5" : "border-transparent text-muted-foreground hover:text-foreground"
            }`}>
            {label}
          </Link>
        ))}
      ...
      <div className="rounded-3xl border border-border bg-card/15 p-6 text-center text-muted-foreground py-16 space-y-2">
        <span className="text-xs font-bold text-foreground block">Webhook Integrations Coming Soon</span>
        <span className="text-[10px]">No active webhooks configured for internal endpoints.</span>
      </div>
    </div>
  );
}
