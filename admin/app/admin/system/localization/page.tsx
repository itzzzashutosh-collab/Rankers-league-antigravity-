  ...
  return (
    <div className="space-y-6 text-foreground animate-fade-in pb-12 font-semibold text-xs">
        ...
        {navLinks.map(({ label, href }) => (
          <Link key={href} href={href}
            className={`px-3 py-2 rounded-t-lg transition-all border-b-2 ${
              href === "/admin/system/localization" ? "border-primary text-primary bg-primary/5" : "border-transparent text-muted-foreground hover:text-foreground"
            }`}>
            {label}
          </Link>
        ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="rounded-3xl border border-border bg-card/15 p-6 space-y-3">
          <h3 className="font-bold text-foreground">Language Support</h3>
          <p className="text-[10px] text-muted-foreground">Default Translation Language: English</p>
        </div>
        <div className="rounded-3xl border border-border bg-card/15 p-6 space-y-3">
          <h3 className="font-bold text-foreground">Regional Currency Format</h3>
          <p className="text-[10px] text-muted-foreground">Default currency settings: INR (₹)</p>
        </div>
      </div>
    </div>
  );
