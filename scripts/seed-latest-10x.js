    ...
    for (const dl of DIFFICULTY_LEVELS) {
      ...
      for (let i = 0; i < stubsNeeded; i++) {
        ...
        if ((dl.level === 'pro' || dl.level === 'legend') && siblings.length > 0) {
          // Merge with 1 or 2 sibling concepts from the same chapter
          const numMerge = Math.min(siblings.length, i === 0 ? 1 : 2);
          ...
          if (dl.level === 'pro') {
            ...
            // legend: reverse thinking + multi-concept
            stem = hasFormula
        ...
        finalTemplates.push({
          ...
          stem_template: stem,
          variables: vars,
          option_a: `Correct option for ${c.concept_name} (${dl.level}): standard application.`,
          option_b: `Incorrect option: misapplies the formula units.`,
          option_c: `Incorrect option: ignores sign orientation.`,
          option_d: `Incorrect option: unrelated conceptual distractor choice.`,
